...
As mentioned above, [Fresh] renders everything into static HTML on the server. On-top of that, when you create API routes in [Fresh], it's based on the [Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) interface allowing it to be deployed to serverless Edge functions such as [Deno Deploy](https://deno.com).

### Island based client hydration

Essentially, the island based client hydration model works with small portions of your application which need JavaScript in-order to be interactive. For example, on the [Fresh homepage](https://fresh.deno.dev/) they have this counter at the bottom which was hydrated to provide interactivity.
...
[Fresh] has absolutely no build step! Unlike a lot of other frameworks, [Fresh] builds can just be directly deployed to the edge with [Deno Deploy](https://deno.com).

...
Moreover, another issue with ORMs is that they try to hide the semantics of relational databases from the developer. Instead of writing explicit joins of various kinds and whatnot, they provide you with a linear API and try to turn that into *somewhat* efficient or [inefficient queries which have terrible performance](https://stackoverflow.com/questions/3763811/linq-queries-in-entity-framework-4-horrible-performance). 

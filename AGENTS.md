...
- Last-visitor / locate features use Upstash Redis: `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`.
- API routes read env via `process.env` (works on Vercel); in local dev fall back to `import.meta.env` when `process.env` is empty.
- Bento layout lives in `src/components/main.astro` with 14 numbered grid slots; cards open external links in new tabs (`target="_blank" rel="noreferrer"`).
- Static assets: bento media under `public/bento/` (mp4s), shared assets like `public/disc.webp` and `public/meme.gif` at the public root.
